...
# --- 3. CSS VISUAL (GRADE FIXA / SCROLL HORIZONTAL) ---
st.markdown("""
<style>
    ...
    /* --- O SEGREDO DO LAYOUT GOOGLE --- */
    /* Força as colunas a ficarem lado a lado, criando scroll se precisar */
    ...
    /* Define largura mínima para as colunas de dia não esmagarem */
    div[data-testid="column"] {
        flex: 0 0 auto !important;
        width: auto !important;
        min-width: 60px !important; /* Largura mínima de cada dia */
    }

    /* Coluna de Hora (a primeira) fica menorzinha */
    ...
    div[data-testid="stForm"] button, button[kind="primary"] {
        background: #0d9488 !important; border: none; color: white !important; font-weight: 600; border-radius: 6px; height: 40px;
    }
    
    /* Botões da Grade (Slots) */
    ...
    /* Chips de Evento */
    .evt-chip {
        background: #ccfbf1; border-left: 3px solid #0d9488; color: #0f766e;
        font-size: 9px; font-weight: 700; height: 45px;
        display: flex; align-items: center; justify-content: center;
        border-radius: 4px; text-align: center; white-space: normal; line-height: 1.1;
    }
    .admin-blocked {
        background: #334155; color: #cbd5e1; font-size: 10px;
        height: 45px; display: flex; align-items: center; justify-content: center; border-radius: 4px;
    }
    .blocked-slot {
        background: repeating-linear-gradient(45deg,#f8fafc,#f8fafc 10px,#f1f5f9 10px,#f1f5f9 20px);
        height: 45px; border-radius: 4px; opacity: 0.5;
    }

    /* Labels de Hora */
    .time-label { font-size: 10px; color: #64748b; text-align: center; margin-top: 15px; font-weight: bold; }
    
    /* Cabeçalhos de Dia */
    .day-header { text-align: center; font-size: 10px; font-weight: 700; color: #334155; padding: 5px 0; border-bottom: 2px solid #e2e8f0; margin-bottom: 5px; }
    .day-num { font-size: 14px; color: #0f172a; display: block; }
    .today-hl { color: #0d9488 !important; border-bottom-color: #0d9488 !important; }

    /* Outros */
    button[help="Excluir Usuário"] { border-color: #fecaca !important; color: #ef4444 !important; background: #fef2f2 !important; }
    div[data-testid="stTextInput"] button { background: transparent !important; border: none; box-shadow: none; }
</style>
...
def resolver_nome(email, nome_meta=None, nome_banco=None):
    ...
    return str(nome_completo).strip().split(' ')[0].title()[:6] # Limita caracteres p/ caber
        ...
        try:
            ...
            for d_res in datas_to_book:
                ...
                for h_start, h_end in horarios_selecionados:
                    ...
                    if dt_check < agora: st.error("Horário passou."); return
                    ...
                    if chk.data: st.error("Já ocupado."); return 
...
def render_calendar(sala, is_admin_mode=False):
    ...
    with c2: st.markdown(f"<div style='text-align:center; font-weight:800; color:#334155; margin-top:5px'>{mes_str} {ref.year}</div>", unsafe_allow_html=True)
    ...
    # --- RENDERIZAÇÃO DA GRADE (ESTILO GOOGLE) ---
    
    # 1. CABEÇALHO (DIAS) - 8 Colunas (1 Hora + 7 Dias)
    ...
    for i, d in enumerate(dias_visiveis):
        with cols_header[i+1]:
            ...
            style_hj = "today-hl" if is_today else ""
            st.markdown(f"<div class='day-header {style_hj}'>{dias_sem[d.weekday()]}<span class='day-num'>{d.day}</span></div>", unsafe_allow_html=True)

    # 2. LINHAS DE HORÁRIO
    for h in range(7, 22):
        row = st.columns(col_ratios)
        
        # Coluna da Hora (Fixa a Esquerda)
        with row[0]:
        ...
        for i, d in enumerate(dias_visiveis):
            with row[i+1]:
                ...
                past = dt_slot < agora
                sunday = d.weekday() == 6
                sat_closed = (d.weekday() == 5 and h >= 14)
                ...
                if res:
                    ...
                    if res['status'] == 'bloqueado':
                        ...
                        if is_admin_mode:
                             if cont.button("🗑️", key=f"blk_{res['id']}"): supabase.table("reservas").update({"status": "cancelada"}).eq("id", res['id']).execute(); st.rerun()
                    else:
                        nm = resolver_nome(res['email_profissional'], nome_banco=res.get('nome_profissional'))
                        if is_admin_mode:
                            c_chip, c_del = cont.columns([3,1])
                            c_chip.markdown(f"<div class='evt-chip'>{nm}</div>", unsafe_allow_html=True)
                            if c_del.button("x", key=f"del_{res['id']}"): supabase.table("reservas").update({"status": "cancelada"}).eq("id", res['id']).execute(); st.rerun()
                        else:
                            cont.markdown(f"<div class='evt-chip'>{nm}</div>", unsafe_allow_html=True)
                
                elif sunday or sat_closed or past:
                    # BLOQUEADO
                    ...
                    if not is_admin_mode:
                        ...
                        cont.markdown("<div style='height:45px; border-left:1px dashed #e2e8f0'></div>", unsafe_allow_html=True)
...
def tela_admin_master():
    ...
    with tabs[0]: 
        ...
        with c1: 
            ...
            pm = st.number_input("Manhã (07-12h)", value=cf['preco_manha'])
            pt = st.number_input("Tarde (13-18h)", value=cf['preco_tarde'])
        with c2:
            pn = st.number_input("Noite (18-22h)", value=cf['preco_noite'])
            pdia = st.number_input("Diária (07-22h)", value=cf['preco_diaria'])
            st.write("")
    ...
    with tabs[2]:
        ...
        if c_b1.button("Bloquear", type="primary", use_container_width=True):
            ...
            try:
                for s in salas:
                    ...
                    inserts = [{"sala_nome": s, "data_reserva": str(dt_block), "hora_inicio": f"{h:02d}:00", "hora_fim": f"{h+1:02d}:00", "user_id": st.session_state.user.id, "email_profissional": st.session_state.user.email, "nome_profissional": "BLOQUEIO", "valor_cobrado": 0, "status": "bloqueado"} for h in range(7, 22)]
                    supabase.table("reservas").insert(inserts).execute()
                st.success("Bloqueado!")
            except Exception as e: st.error(f"Erro: {e}")
            
        if c_b2.button("Desbloquear", type="secondary", use_container_width=True):
            salas = ["Sala 1", "Sala 2"] if sala_block == "Ambas" else [sala_block]
            try:
    ...
    with tabs[3]:
        ...
        if st.button("Gerar Extrato", type="primary"): 
            # (Lógica simplificada de relatório mantida da V86)
            st.info("Funcionalidade de PDF disponível.")
    with tabs[4]:
        # Usuários
        st.info("Gestão de usuários disponível (service_key).")
    ...
    if not st.session_state.user:
        ...
        with c2:
            ...
            if st.session_state.auth_mode == 'login':
                ...
                if c_b.button("Esqueci Senha"): st.session_state.auth_mode = 'forgot'; st.rerun()
                ...
                if st.button("Cadastrar", type="primary", use_container_width=True):
                    try: supabase.auth.sign_up({"email": em, "password": pw, "options": {"data": {"nome": nm}}}); st.success("Sucesso!"); time.sleep(1); st.session_state.auth_mode='login'; st.rerun()
                    except: st.error("Erro")
            ...
            try:
                ...
                if not df.empty:
                    for _, row in df.iterrows():
                        ...
                        if dt_res < agora:
                            st.markdown(f"<div style='background:#f8fafc; padding:10px; border-radius:8px; border:1px solid #e2e8f0; color:#94a3b8; margin-bottom:10px'>✅ <strong>{row['data_reserva']}</strong> às {row['hora_inicio'][:5]} <small>({row['sala_nome']})</small></div>", unsafe_allow_html=True)
                        else:
                            with st.container():
