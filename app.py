...
<style>
    ...
    /* Isso garante que os dias fiquem lado a lado mesmo no celular */
    ...
    .evt-card-desktop {
        ...
        height: 42px; line-height: 1.2; display: flex; align-items: center;
    }

    /* Estilo do Indicador Mobile (Pílula/Dot) */
    ...
    @media (max-width: 768px) {
        /* Inverte a visibilidade: Esconde texto, mostra pílula */
        ...
        /* Botões do Streamlit ficam invisíveis mas clicáveis */
    ...
    .day-name { font-size: 11px; font-weight: 600; color: #64748b; text-transform: uppercase; }
    ...
    /* Login Clean */
    .login-container { padding: 20px; border-radius: 10px; border: 1px solid #f1f5f9; margin-top: 20px; }
</style>
...
def navegar(direcao):
    delta = 7 if st.session_state.view_mode == 'SEMANA' else 1
...
def render_calendar(sala, is_admin_mode=False):
    ...
    if mode == 'MÊS':
        # (Modo mês simplificado para focar na semana híbrida)
        mes_nome = ref.strftime("%B %Y").upper()
        c2.markdown(f"<div style='text-align:center; font-weight:bold; margin-top:5px'>{mes_nome}</div>", unsafe_allow_html=True)
        # ... (implementação padrão de mês)
        st.info("Visualização Mensal indisponível no modo híbrido. Use 'Semana'.")
        if st.button("Voltar para Semana"): st.session_state.view_mode = 'SEMANA'; st.rerun()
        return

    # LÓGICA SEMANA HÍBRIDA
    ...
    # O CSS vai forçar isso a ser um GRID RÍGIDO
    # ratios = [0.3] + [1] * 7  -> CORREÇÃO AQUI (num_days era o erro)
    ratios = [0.3, 1, 1, 1, 1, 1, 1, 1]
    ...
    # 2. Linhas (Horas)
    for h in range(7, 22):
        row = st.columns(ratios) # Isso vira GRID no CSS
...
def main():
    if not st.session_state.user:
        # TELA DE LOGIN (Sem usar st.columns complexas para evitar quebra no mobile)
    ...
    if st.session_state.is_admin:
        ...
        render_calendar(sala, is_admin_mode=True)
        ...
        render_calendar(sala, is_admin_mode=False)
