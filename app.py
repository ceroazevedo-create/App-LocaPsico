...
<style>
    ...
    .filter-box { background: #f1f5f9; padding: 15px; border-radius: 10px; margin-bottom: 20px; }

    /* CALENDÁRIO USER (CSS Mantido para o usuário comum) */
...
def tela_admin_master():
    ...
    # 2. CANCELAMENTO GERAL
    with tabs[1]:
        st.markdown("<div class='filter-box'>", unsafe_allow_html=True)
        st.write("Filtrar Reservas Ativas (Todas)")
        search = st.text_input("Buscar por Nome ou Email")
        st.markdown("</div>", unsafe_allow_html=True)
        
        try:
            # Pega todas as confirmadas
            ...
            if not df.empty:
                ...
                for idx, row in df.iterrows():
                    ...
                    with st.container():
                        ...
                        # Botão Cancelar Direto (Sem verificação de tempo)
    ...
    with tabs[2]:
        ...
        st.info("Selecione o Mês e o Usuário para ver o total e baixar o PDF.")
        
        col_m, col_u = st.columns(2)
        mes_sel = col_m.selectbox("Selecione Mês", ["2026-01", "2026-02", "2026-03", "2026-04", "2026-05", "2026-06"])
        
        # Carregar Usuários que tem reservas
        try:
...
def main():
    if 'user' not in st.session_state:
        ...
        with c2:
            ...
            t_log, t_cad = st.tabs(["Entrar", "Criar"])
            with t_log:
