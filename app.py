...
# --- 3. CSS VISUAL (ULTRA DENSITY MOBILE MODE) ---
st.markdown("""
<style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&display=swap');
    
    .stApp { background-color: #ffffff; font-family: 'Inter', sans-serif; color: #1e293b; }
    .block-container { padding-top: 1rem !important; max-width: 1200px; padding-bottom: 2rem; }

    /* ============================================================ */
    /* ☢️ ZONA DE COMPACTAÇÃO AGRESSIVA (MOBILE < 768px) ☢️      */
    /* Objetivo: Densidade Google Agenda. Zero desperdício.         */
    /* ============================================================ */
    
    @media only screen and (max-width: 768px) {
        
        /* 1. CONTAINER PAI (O GRID) */
        /* Força scroll lateral e proíbe quebra de linha */
        div[data-testid="stHorizontalBlock"] {
            display: flex !important;
            flex-wrap: nowrap !important;
            overflow-x: auto !important;
            gap: 1px !important; /* Espaço mínimo entre colunas */
            padding-bottom: 10px; /* Para o dedo rolar */
            align-items: stretch !important;
        }

        /* 2. COLUNAS ANORÉXICAS */
        div[data-testid="column"] {
            flex: 0 0 auto !important;
            width: auto !important;
            min-width: 48px !important; /* Largura estreita para caber muitos dias */
            padding: 0 1px !important;  /* Quase zero padding lateral */
        }

        /* 3. COLUNA DA HORA (EIXO Y) - STICKY */
        div[data-testid="column"]:first-child {
            position: sticky !important;
            left: 0 !important;
            background-color: #ffffff !important;
            z-index: 50 !important; /* Fica por cima ao rolar */
            min-width: 35px !important; /* Bem fininha */
            border-right: 1px solid #e2e8f0 !important;
        }

        /* 4. CÉLULAS E BOTÕES (SLOTS) - COMPACTAÇÃO VERTICAL */
        /* Afeta botões, markdown e containers dentro da coluna */
        div[data-testid="stVerticalBlock"] button,
        div[data-testid="stVerticalBlock"] div[data-testid="stMarkdown"], 
        .evt-chip, .admin-blocked, .blocked-slot {
            min-height: 30px !important; /* Altura fixa pequena */
            height: 30px !important;
            padding: 0 !important;       /* Zero padding interno */
            margin: 1px 0 !important;    /* Mínima margem vertical */
            border-radius: 2px !important; /* Cantos pouco arredondados (estilo técnico) */
            line-height: 30px !important; /* Centraliza texto verticalmente */
        }
        
        /* Ajuste específico para o botão nativo do Streamlit */
        ...
        p, button, div, span {
            font-size: 10px !important; /* Fonte muito pequena */
            white-space: nowrap !important; /* Não quebra linha */
            overflow: hidden !important;    /* Corta o excedente */
            text-overflow: ellipsis !important; /* Adiciona "..." */
        }
        
        /* Ajuste do label da hora */
        .time-label {
            font-size: 9px !important;
            transform: translateY(-8px); /* Sobe um pouco para alinhar */
        }
        
        /* Remove cabeçalho e rodapé para ganhar espaço */
        header { display: none !important; }
        footer { display: none !important; }
    }
    /* ============================================================ */

    /* --- ESTILOS DESKTOP (MANTIDOS) --- */
    div[data-testid="column"]:nth-of-type(2) > div { background-color: #ffffff; padding: 30px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); border: 1px solid #f1f5f9; }
    div[data-testid="stImage"] { display: flex; justify-content: center; margin-bottom: 20px; }
    div[data-testid="stImage"] > img { max-height: 90px; object-fit: contain; }
    h1 { font-size: 22px; font-weight: 700; color: #0f172a; text-align: center; }
    
    .stTextInput input { background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; height: 42px; }
    div[data-testid="stForm"] button, button[kind="primary"] { background: linear-gradient(180deg, #0f766e 0%, #0d9488 100%) !important; border: none; height: 42px; border-radius: 6px; color: white; font-weight: 600; }
    button[kind="secondary"] { background-color: white; border: 1px solid #cbd5e1; color: #475569; border-radius: 6px; height: 42px; }
    ...
    @media (min-width: 769px) {
        ...
        .evt-chip { height: 40px; padding: 0 8px; font-size: 11px; }
    }
    
    /* Chips e Status Comuns */
    .evt-chip { background: white; border-left: 3px solid #0d9488; color: #0f766e; font-weight: 600; display: flex; align-items: center; justify-content: center; overflow: hidden; }
    .admin-blocked { background: #334155; color: #cbd5e1; display: flex; align-items: center; justify-content: center; font-weight: bold; }
    .blocked-slot { background-color: #f8fafc; opacity: 0.6; }
...
def render_calendar(sala, is_admin_mode=False):
    ...
    with c1: 
        if st.button("◀", use_container_width=True, key=f"nav_prev_{is_admin_mode}"): navegar('prev'); st.rerun()
    with c3: 
        if st.button("▶", use_container_width=True, key=f"nav_next_{is_admin_mode}"): navegar('next'); st.rerun()
    ...
    if mode == 'MÊS':
        ...
        # (Lógica mês mantida, mas CSS cuida do layout)
        # ... Para brevidade, mantive o foco na semana, que é o crítico ...
        
    else:
        # LÓGICA DA SEMANA / DIA
        if mode == 'SEMANA':
            ...
            d_start = ref
            d_end = ref
            lbl = f"{ref.day} {mes_str}"
            num_dias = 1
            
        st.markdown(f"<div style='text-align:center; font-weight:800; color:#334155; margin:5px 0; font-size:14px'>{lbl}</div>", unsafe_allow_html=True)
        ...
        # 1. CABEÇALHO (AGORA COMPACTO)
        # Ratios: Hora (pequena) + Dias
        ratios = [0.4] + [1]*num_days
        ...
        for i, d in enumerate(visiveis):
            ...
            cols[i+1].markdown(f"<div style='text-align:center; border-bottom:1px solid #e2e8f0; padding-bottom:2px'><div style='font-size:9px; font-weight:bold; color:#64748b'>{d_n[wd]}</div><div style='font-size:12px; font-weight:bold; color:#1e293b'>{d.day}</div></div>", unsafe_allow_html=True)

        # 2. GRADE DE HORÁRIOS (SLOTS DE 30PX)
        for h in range(7, 22):
                    ...
                    if res:
                        if res['status'] == 'bloqueado':
                            ...
                            if is_admin_mode:
                                ...
                                c1.markdown(f"<div class='evt-chip' title='{nm}'>{nm}</div>", unsafe_allow_html=True)
                                if c2.button("x", key=f"del_{res['id']}"): supabase.table("reservas").update({"status": "cancelada"}).eq("id", res['id']).execute(); st.rerun()
                            else:
                                cont.markdown(f"<div class='evt-chip' title='{nm}'>{nm}</div>", unsafe_allow_html=True)
                        ...
                        if not is_admin_mode:
                            ...
                            cont.markdown("<div style='height:30px; border-left:1px dashed #e2e8f0'></div>", unsafe_allow_html=True)
